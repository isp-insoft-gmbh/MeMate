...
/**
 * Der {@link DrinkManagerDialog} erzeugt einen neuen Frame, wenn im {@link Drinkmanager} ein Getränk hinzugefügt oder
 * bearbeitet wird. In diesem Frame kann man den Name, Preis, und ein Bild des Getränks angeben oder editieren
 * 
 * @author nwe
 * @since 18.10.2019
 */
public class DrinkManagerDialog
{
    ...
    public DrinkManagerDialog(Window owner)
    {
        ...
        fileChooserButton.addActionListener(new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent __)
            {
                String picturePath = "C:" + File.separator + "Users" + File.separator + "admlokal" + File.separator
                    + "Pictures" + File.separator + "MeMateImages" + File.separator;
                fileChooser.showOpenDialog(owner);
                String filename = fileChooser.getSelectedFile().getName();
                drinkPictureField.setText(filename);
                File selectedFile = fileChooser.getSelectedFile();
                File copiedFile = new File(picturePath + filename);
                try {
                    Files.copy(selectedFile.toPath(), new FileOutputStream(copiedFile));
                } catch (IOException exception) {
                    System.out.println("Bild konnte nicht kopiert werden");
                }
            }
        });
    ...
    public void showEditDialog(String drink)
    {
        ...
        drinkPictureField
            .setText(ServerCommunication.getInstance().getIcon(drink).toString().replace("T:\\NWE\\MeMate\\", ""));
        drinkPriceSpinner.setValue(ServerCommunication.getInstance().getPrice(drink));
    ...
    public void showNewDialog()
    {
        ...
        confirmButton.addActionListener(new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent __)
            {
                ...
                String picture = drinkPictureField.getText();
                String priceAsString = String.valueOf(drinkPriceSpinner.getValue());
                Float price = Float.valueOf(priceAsString);
                if (name.isEmpty()) {
                    JOptionPane.showConfirmDialog(dialog, "Bitte Getränkenamen eingeben");
                } else if (picture.isEmpty()) {
                    JOptionPane.showConfirmDialog(dialog, "Bitte Bild des Getränks auswählen");
                } else if (price.equals(0f)) {
                    JOptionPane.showConfirmDialog(dialog, "Bitte Preis des Getränks angeben");
                } else {
                    ServerCommunication.getInstance().registerNewDrink(name, price, picture);
                    dialog.dispose();
                }
            }
        });
